% function to estimate the target power and Intersite Phase Clustering (ISPC) or equivalently, the Phase Locking Value (PLV), within event-marked communication windows
function [Tpow,Apow,Bpow,Glopow,Ghipow,PLV_theta,PLV_alpha,PLV_beta,PLV_gammalo,PLV_gammahi] = neural_osc_network(data,bandpassSig,SC,win,N,delay,sigevents,sampling_rate)
    Tpow=zeros(N,N);
    Apow=zeros(N,N);
    Bpow=zeros(N,N);
    Glopow=zeros(N,N);
