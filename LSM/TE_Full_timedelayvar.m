...
TE = zeros(N,N,length(del));
TE_std = zeros(N,N,length(del));
for p=1:length(del)
    p
    ...
    for rep=1:repeat
        ...
        %% MI analysis
        delay=floor([0 d12 nan;d12 0 d32+del(p);nan d32+del(p) 0]*Fs);
...
figure;
hold on
shadedErrorBar(del,smooth(squeeze(TE(1,2,:))',smoothfactor),smooth(squeeze(TE_std(1,2,:))'/sqrt(repeat),smoothfactor),'lineProps','r')
shadedErrorBar(del,smooth(squeeze(TE(2,1,:))',smoothfactor),smooth(squeeze(TE_std(2,1,:))'/sqrt(repeat),smoothfactor),'lineProps','g')
shadedErrorBar(del,smooth(squeeze(TE(2,3,:))',smoothfactor),smooth(squeeze(TE_std(2,3,:))'/sqrt(repeat),smoothfactor),'lineProps','k')
shadedErrorBar(del,smooth(squeeze(TE(3,2,:))',smoothfactor),smooth(squeeze(TE_std(3,2,:))'/sqrt(repeat),smoothfactor),'lineProps','b')
